...
public class GlobalExceptionHandler {
    ...
    public ApiError handleHttpMessageNotReadableException(HttpMessageNotReadableException e) {
        ...
        return new ApiError(
                "BAD_REQUEST",
                "Переданы некорректные данные.",
                "Некорректно составлен запрос.",  // ← БЕЗ стэк-трейса!
                LocalDateTime.now()
        );
    }
    ...
    public ApiError handleException(Exception e) {
        ...
        // ВАЖНО: ТОЛЬКО для 500 ошибок - стэк-трейс оставить!
        String stackTrace = getStackTrace(e);
        String message = "Произошла непредвиденная ошибка. Администратор уже уведомлён.\n\n" + stackTrace;
        ...
        return new ApiError(
                "INTERNAL_ERROR",
                "Внутренняя ошибка сервера.",
                message,  // ← СО стэк-трейсом для 500!
                LocalDateTime.now()
        );
    }
